pipeline {
    ...
    stages {
        stage('fetch'){
            steps{
                pwd()
                git branch: 'dev', url: 'https://github.com/Utopia-Airline/customer-portal.git'
            }
        }
        stage('build'){
            steps{
                ...
                sh "aws ecs update-service --cluster utopiaCluster --service web-app-service --force-new-deployment"               
            }
        }
    }
}